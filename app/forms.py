...
def check_for_len(v):
    if len(v) < 5:
        raise forms.ValidationError('lenght issues')


class StudentForm(forms.Form):
    name = forms.CharField(max_length=100, validators=[
                           check_for_a, validators.MaxLengthValidator(5)])
    botcatcher = forms.CharField(
        max_length=100, widget=forms.HiddenInput, required=False)
    email = forms.EmailField(max_length=100)
    reenter_email = forms.EmailField(max_length=100)
    mobile = forms.CharField(max_length=10, min_length=10, validators=[
                             validators.RegexValidator('[6-9]\d{9}')])
